...
Если нет желания самостоятельно прошивать модуль через esptool, можно воспользоваться готовым дампом всей флеш памяти модуля (все 4мб). Файл [MksWifi.bin](./fixed_flash/MksWifi.bin) можно просто положить на SD карту и включить принтер. Поскольку загружается образ всей флеш памяти, а не только прошивка, процесс занимает несколько больше времени.
